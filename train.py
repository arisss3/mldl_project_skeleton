...
tiny_imagenet_dataset_train = ImageFolder(root='tiny-imagenet-200/train', transform=transform)
tiny_imagenet_dataset_val = ImageFolder(root='tiny-imagenet-200/val', transform=transform)
...
if os.path.exists(val_images_path):  # Check if the directory exists
    with open('tiny-imagenet-200/val/val_annotations.txt') as f:
        for line in f:
            fn, cls, *_ = line.split('\t')
            os.makedirs(f'tiny-imagenet-200/val/{cls}', exist_ok=True)

            shutil.copyfile(f'{val_images_path}/{fn}', f'tiny-imagenet-200/val/{cls}/{fn}')
